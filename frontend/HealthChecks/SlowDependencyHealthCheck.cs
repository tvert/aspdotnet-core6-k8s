...
namespace GloboTicket.Frontend.HealthChecks
{
    public class SlowDependencyHealthCheck : IHealthCheck
    {
        ...
        {
            if (_invocationCount++>3)
            {
                return Task.FromResult(HealthCheckResult.Healthy($"Dependency is ready. Invocation count:{_invocationCount}"));
            }
            
            return Task.FromResult(new HealthCheckResult(
                status: context.Registration.FailureStatus,
                description: $"Dependency is still initializing. Invocation Count:{_invocationCount}"));
        }
    }
}
